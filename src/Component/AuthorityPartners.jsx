      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 sm:gap-12">
          {partners.map((partner) => (
            <div
              key={partner.id}
              className="flex items-start gap-4 sm:gap-5 bg-white p-5 rounded-lg shadow hover:shadow-lg transition-shadow duration-300"
            >
              <div className="text-primary text-3xl mt-1 shrink-0">
                {partner.icon}
              </div>
              <div>
                <h3 className="font-medium text-lg mb-1">{partner.title}</h3>
